...
// connTest func 
func connTest() (*pgxpool.Pool, error) {
	pCfg, err := pgxpool.ParseConfig(fmt.Sprintf("host=%v port=%v dbname=%v user=%v password=%v", "localhost", "5432", "deputychallenge", "deputyuser", "password"))
	if err != nil {
		return nil, err
	}
